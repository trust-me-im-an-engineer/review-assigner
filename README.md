...
ErrorResponse определяет возвращаемую структуру ошибок, но содержит не достаточно полей в code enum: отсутствует код для невалидного запроса (например `BAD_REQUEST`) 

#### Допущение

Так как openapi схема считается неизменяемой, для невалидных запросов использован код ошибки `NOT_FOUND`, хоть это и семантически некорректная ошибка.